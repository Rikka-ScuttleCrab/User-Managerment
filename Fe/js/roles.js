 const {
  ...
  async function createRole() {
    ...
    try {
      ...
      const result = await res.json();
        if (!res.ok) {
          if(result.message === "Role already exists")
            throw new Error("Tên nhóm này đã tồn tại");
          else throw new Error (result.message || "Tạo nhóm thất bại")
        }
  ...
  async function loadAllUsers() {
    ...
    const response = await apiFetch(
      `${API_BASE}/v1/api/admin/users/search`
    );
