...
async function loadPermissionsPage() {
  try {
    ...
    renderRoles();
    if (allRoles.length > 0 && selectedRole === null) {
      selectRole(
        allRoles[0].roleName
      );
    }
    else if(selectedRole !== null) {
      selectRole(selectedRole.roleName);
    }
  } catch (err) {
    console.log(err);
    adminlogout();
...
function renderPermissionList() {
    ...
    container.innerHTML += `
      <label class="permission-card">
        <input
          ...
          ${checked ? "checked" : ""}
          ${isAdmin ? "disabled" : ""}
          data-module="${permission.permissionName}"
          data-action="${permission.permissionAction}"
  ...
  document
    .querySelectorAll(
        '.permission-checkbox[data-action="VIEW"]'
    )
    .forEach(view => {
...
function syncPendingPermissions() {
        ...
        .forEach(cb => {
            ...
            if (cb.checked) {
                ...
                pendingPermissions.push(
                    `${cb.dataset.module}-${cb.dataset.action}`
                );
...
function handlePermissionChange(checkbox) {
    ...
    // Nếu tick GET thì tự tick VIEW
    if (action === "GET" && checkbox.checked) {
    ...
    if (action === "VIEW" && !checkbox.checked) {
        ...
        document.querySelectorAll(
            ...
            if (
                cb.dataset.action !== "VIEW" &&
                cb.dataset.action !== "GET"
            ) {
                cb.checked = false;
                cb.disabled = true;
            }
    ...
    // Tick CREATE/UPDATE/DELETE/ROLE...
    if (
        action !== "VIEW" &&
        action !== "GET" &&
        checkbox.checked
